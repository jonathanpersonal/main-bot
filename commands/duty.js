...
async function handleActivityReport(interaction) {
  ...
  if (!dryRun && activityConfig.reportChannelId) {
    const reportChannel = await interaction.guild.channels.fetch(activityConfig.reportChannelId).catch(() => null);
    if (reportChannel && typeof reportChannel.send === 'function') {
      const message = await reportChannel.send({ embeds: [report.embed] });
      report.cycle.reportMessageId = message.id;
      report.cycle.reportChannelId = message.channel.id;
      await saveActivityCycle(report.cycle);
    }
  }
  ...
  if (!dryRun && activityConfig.commandReviewChannelId && reviewFindings.length > 0) {
    const reviewChannel = await interaction.guild.channels.fetch(activityConfig.commandReviewChannelId).catch(() => null);
  ...
  return interaction.editReply({ embeds: [report.embed] });
}
