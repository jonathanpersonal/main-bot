...
async function handleActivityReport(interaction) {
  ...
  try {
    ...
    if (!dryRun) {
      ...
      }).catch((error) => console.warn('[activity-report] duty log skipped:', error.message || error));
    }

    if (!dryRun && activityConfig.reportChannelId) {
      const reportChannel = await interaction.guild.channels.fetch(activityConfig.reportChannelId).catch((error) => {
      ...
      if (reportChannel && typeof reportChannel.send === 'function') {
        const message = await reportChannel.send({ embeds: [report.embed] }).catch((error) => {
    ...
    if (!dryRun && activityConfig.commandReviewChannelId && reviewFindings.length > 0) {
      const reviewChannel = await interaction.guild.channels.fetch(activityConfig.commandReviewChannelId).catch((error) => {
      ...
      if (reviewChannel && typeof reviewChannel.send === 'function') {
        ...
        await reviewChannel.send(`Activity command review required for ${reviewFindings.length} officer(s):\n${reviewLines.join('\n')}`).catch((error) => console.warn('[activity-report] command review post skipped:', error.message || error));
      }
    }
